...
const fontStyle = 'px Times New Roman';
...
export default class Graph {
     ...
     */
    public setBarWidthPercent(percent: number): void {
        this.barWidthPercent = percent;
    }

    public setGraphDimensions(graphDimensions: GraphDimensions): void {
    ...
    public drawGraph(): void {
        ...
        this.context.font = `${this.graphDimensions.height / 30}${fontStyle}`;
        ...
        // Draw graph title
        this.context.font = `${this.graphDimensions.height / 20}${fontStyle}`;
    ...
    private drawXAxis(): void {
        ...
        const interval = width / (this.data.data.length + 1);
        ...
        for (let i = 0; i < this.data.data.length; i++) {
            ...
            context.font = `${interval / 5}${fontStyle}`;
    ...
    private drawYAxis(): void {
        ...
        const interval = this.barMaxHeight / data.maxData;
        context.font = `${interval}${fontStyle}`;
        ...
        for (let i = 0; i < data.data.length; i++) {
            ...
            const calculateRelativeHeight = interval * time[time.length - 1].time;
            context.beginPath();
        ...
        for (let i = 0; i < data.maxData + 1; i++) {
    ...
    private drawBar(value: Types.LanguageStat, barProps: BarProps): void {
        if (!this.context) return;
        const context = this.context;
        ...
        // calculate interval
        const interval = barProps.height / this.data.maxData;
        // Interval
        const calculateRelativeHeight = interval * value.time[value.time.length - 1].time;
        ...
        function moveEvent(event: MouseEvent): void {
            const locationX = (event.offsetX / context.canvas.offsetWidth) * context.canvas.width;
            const locationY = (event.offsetY / context.canvas.offsetHeight) * context.canvas.height;
    ...
    private drawInfoBox(timeObject: Types.TimeObject): void {
        if (!this.context) return;
        const context = this.context;

        context.save();
        // Get last changed banner
        const date = new Date(timeObject.Created_date);
        const outputString = `Last Changed: ${date.getMonth() + 1}/${date.getDate()}/${date.getFullYear()}`;

        context.textAlign = 'left';

        // Get the size of the text and add padding
        const textWidth = context.measureText(outputString).width + 10;
