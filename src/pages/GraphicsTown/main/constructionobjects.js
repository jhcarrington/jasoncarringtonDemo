...
export class GrCrane extends GrObject {
  ...
  constructor (params = {}) {
    ...
    base_curve.lineTo(-0.5, 0);
    const base_geom = new T.ExtrudeGeometry(base_curve, exSettings)
    const crane_mat = new T.MeshStandardMaterial({
      color: 'yellow',
      metalness: 0.5,
      roughness: 0.7
    })
    const base = new T.Mesh(base_geom, crane_mat)
    crane.add(base);
    ...
    const arm_group = new T.Group()
    crane.add(arm_group);
    arm_group.translateY(4.5);
    const arm_curve = new T.Shape()
    ...
    const arm_geom = new T.ExtrudeGeometry(arm_curve, exSettings)
    const arm = new T.Mesh(arm_geom, crane_mat)
    arm_group.add(arm);
    arm.translateZ(-0.25);
    ...
    const wire_group = new T.Group()
    arm_group.add(wire_group);
    wire_group.translateX(3);
    const wire_curve = new T.Shape()
    ...
    wire_curve.lineTo(-0.25, 0);
    const wire_geom = new T.ExtrudeGeometry(wire_curve, exSettings)
    const wire_mat = new T.MeshStandardMaterial({
      color: '#888888',
      metalness: 0.6,
      roughness: 0.3
    })
    const wire = new T.Mesh(wire_geom, wire_mat)
    wire_group.add(wire);
...
export class GrExcavator extends GrObject {
  ...
  constructor (params = {}) {
    ...
    base_curve.lineTo(-1, 0);
    const base_geom = new T.ExtrudeGeometry(base_curve, exSettings)
    const excavator_mat = new T.MeshStandardMaterial({
      color: 'yellow',
      metalness: 0.5,
      roughness: 0.7
    })
    const base = new T.Mesh(base_geom, excavator_mat)
    excavator.add(base);
    base.translateZ(-0.2);
    ...
    const pedestal_curve = new T.Shape()
    pedestal_curve.moveTo(-0.35, 0);
    pedestal_curve.lineTo(-0.35, 0.25);
    pedestal_curve.lineTo(0.35, 0.25);
    pedestal_curve.lineTo(0.35, 0);
    pedestal_curve.lineTo(-0.35, 0);
    const pedestal_geom = new T.ExtrudeGeometry(pedestal_curve, exSettings)
    const pedestal = new T.Mesh(pedestal_geom, excavator_mat)
    ...
    const cab_group = new T.Group()
    excavator.add(cab_group);
    cab_group.translateY(0.7);
    const cab_curve = new T.Shape()
    ...
    cab_curve.lineTo(-1, 0);
    const cab_geom = new T.ExtrudeGeometry(cab_curve, exSettings)
    const cab = new T.Mesh(cab_geom, excavator_mat)
    cab_group.add(cab);
    cab.translateZ(-0.2);
    ...
    const arm_geom = new T.ExtrudeGeometry(arm_curve, exSettings)
    const arm_mat = new T.MeshStandardMaterial({
      color: '#888888',
      metalness: 0.6,
      roughness: 0.3
    })
    const arm = new T.Mesh(arm_geom, arm_mat)
    arm_group.add(arm);
    arm.translateZ(-0.2);

    const forearm_group = new T.Group()
    arm_group.add(forearm_group);
    forearm_group.position.set(-2.1, 0, 0);
    const forearm_curve = new T.Shape()
    ...
    const forearm_geom = new T.ExtrudeGeometry(forearm_curve, exSettings)
    const forearm = new T.Mesh(forearm_geom, arm_mat)
    forearm_group.add(forearm);
    forearm.translateZ(-0.2);

    const bucket_group = new T.Group()
    forearm_group.add(bucket_group);
    bucket_group.position.set(-1.4, 0, 0);
    const bucket_curve = new T.Shape()
    ...
    const bucket_geom = new T.ExtrudeGeometry(bucket_curve, exSettings)
    const bucket = new T.Mesh(bucket_geom, arm_mat)
...
export class GrForkLift extends GrObject {
  ...
  constructor (params = {}) {
    ...
    const base_geom = new T.ExtrudeGeometry(base_curve, {
      steps: 2,
      depth: 0.2,
      bevelEnabled: true,
      bevelThickness: 0.2,
    ...
    const tireMaterial = new T.MeshStandardMaterial({ color: '#0f0f0f' });
    const tire1 = new T.Mesh(base_geom, tireMaterial)
    const tire2 = new T.Mesh(base_geom, tireMaterial)
    const tire3 = new T.Mesh(base_geom, tireMaterial)
    const tire4 = new T.Mesh(base_geom, tireMaterial)
    ...
    pedestal_curve.lineTo(-0.35, 0);
    const pedestal_geom = new T.ExtrudeGeometry(pedestal_curve, exSettings)
    const pedestal = new T.Mesh(pedestal_geom, new T.MeshStandardMaterial({ color: '#050505' }))
    ...
    const leftArm = new T.ExtrudeGeometry(arm, armSettings)
    const rightArm = new T.ExtrudeGeometry(arm, armSettings)
    const innerRackGeom = new T.ExtrudeGeometry(innerRail, exSettings)
    const outerRackGeom = new T.ExtrudeGeometry(outerRail, exSettings)
    const rackMat = new T.MeshStandardMaterial({
      color: '#888888',
      metalness: 0.6,
      roughness: 0.3
    })
    const innerRack = new T.Mesh(innerRackGeom, rackMat)
    const outerRack = new T.Mesh(outerRackGeom, rackMat)
    const leftArmMesh = new T.Mesh(leftArm, rackMat)
    const rightArmMesh = new T.Mesh(rightArm, rackMat)
    const armExtensionGroup = new T.Group()
    ...
    const outerRackGroup = new T.Group()
...
export class GrDumpTruck extends GrObject {
  ...
  constructor (params = {}) {
    ...
    cab_curve.lineTo(-0.25, 0.45);
    cab_curve.lineTo(-0.25, 1.5);
    cab_curve.lineTo(-1.3, 1.5);
    cab_curve.lineTo(-1.5, 1.2);
    cab_curve.lineTo(-1.5, 0);
    const cab_geom = new T.ExtrudeGeometry(cab_curve, exSettings)
    ...
    const bucketGeom = new T.ExtrudeGeometry(bucket, {
      steps: 2,
      depth: 1,
      bevelEnabled: true,
      bevelThickness: 0.2,
    ...
    const sideBucketGeom = new T.ExtrudeGeometry(sideBucket, {
      steps: 2,
      depth: 1,
      bevelEnabled: true,
      bevelThickness: 0.2,
    ...
    const bottomBucketMesh = new T.Mesh(bucketGeom, rackMat)
    const rightBucketMesh = new T.Mesh(bucketGeom, rackMat)
    const leftBucketMesh = new T.Mesh(bucketGeom, rackMat)
    const nearWallMesh = new T.Mesh(sideBucketGeom, rackMat)
