...
export class Car extends GrObject {
  constructor (color, roadPoints) {
    ...
    const windowMesh = new T.Mesh(windows, new MeshStandardMaterial({ color: '#555555' }))
...
function addWindows (height, length, width) {
  ...
  hull.vertices.push(new T.Vector3(-((length + offset) / 2) + (height * 0.19), height / 4, width / 2));
  // bottom left back 13
  hull.vertices.push(new T.Vector3(-((length + offset) / 2) + (height * 0.19), height / 4, -width / 2));
  // top right back 14
  hull.vertices.push(new T.Vector3(-(length + offset) / 4, height, width / 2));
