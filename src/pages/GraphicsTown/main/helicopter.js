...
export class helicopter extends GrObject {
  constructor (scale) {
    ...
    const rotor = this.buildrotor(new T.MeshStandardMaterial({ color: '#000000' }), scale)
  ...
  tick () {
    ...
    const tailRotor = this.objects[0].children.find((obj) => obj.name == 'tail').children.find((obj) => obj.name == 'rotor');
    ...
    const rotation = this.calculateRotation({ x: this.objects[0].position.x - x, y: this.objects[0].position.z - z })
  ...
  buildrotor (material, scale) {
    /**
         ...
         */
    function buildBlade () {
      const blade = new T.CylinderBufferGeometry(scale / 10, scale / 10, scale * 2)
