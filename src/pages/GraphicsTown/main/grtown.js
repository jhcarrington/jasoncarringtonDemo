...
export function grtown (renderer, canvasContainer) {
  ...
  heli1.tick = (delta) => {
    ...
    const tailRotor = heli1.objects[0].children.find((obj) => obj.name == 'tail').children.find((obj) => obj.name == 'rotor');
...
function setPositionUsed (x, y, tree) {
  ...
  usedTreePositions.push({ vector: freeTreePositions.splice(indexOf, 1)[0], tree });
}
/**
 * O(N)
 *
...
function buildRoad (world) {
  ...
  let backRoadPoints = [];
  for (var i = -1; i < 5; i++) {
    const roadMesh = new T.Mesh(roadBlock, new T.MeshStandardMaterial({ map: mainRoad }))
    const sideMeshR = new T.Mesh(sideBlock, new T.MeshStandardMaterial({ map: roadSide }))
    const sideMeshL = new T.Mesh(sideBlock, new T.MeshStandardMaterial({ map: roadSide }))
  ...
  const roadMesh = new T.Mesh(roadBlock, new T.MeshStandardMaterial({ map: roadTurn }))
  const sideMeshR = new T.Mesh(sideBlock, new T.MeshStandardMaterial({ map: roadSide }))
  const sideMeshL = new T.Mesh(sideBlock, new T.MeshStandardMaterial({ map: roadSide }))
  ...
  const sideMeshLE1 = new T.Mesh(sideBlock, new T.MeshStandardMaterial({ map: roadSide }))
  sideMeshLE1.translateX(-5 * 3.5);
  sideMeshLE1.translateZ(8);
  groupSideL.add(sideMeshLE1);
  const sideMeshLE2 = new T.Mesh(new T.CubeGeometry(3, 0.05, 2.5), new T.MeshStandardMaterial({ map: roadSide }))
...
function buildRoadLoop (roadBlock) {
  ...
  const roadMeshSplit = new T.Mesh(roadBlock, new T.MeshStandardMaterial({ map: roadSplit }))
  ...
  for (let i = 0; i < 4; i++) {
    const roadMesh1 = new T.Mesh(roadBlock, new T.MeshStandardMaterial({ map: roadTurn }))
  ...
  const roadMeshFinish = new T.Mesh(roadBlock, new T.MeshStandardMaterial({ map: mainRoad }))
...
function buildTrainTrack (world) {
  ...
  trainTrackPoints.forEach((/** @type {T.Vector3} */ point, index) => {
    const mesh = new T.Mesh(cube, new T.MeshStandardMaterial({ color: '#ff0000' }))
  ...
  const trackMesh = new T.Line(bufferPoints, new T.MeshStandardMaterial({ color: '#0000ff' }));
  const trackMesh2 = new T.Line(bufferPoints2, new T.MeshStandardMaterial({ color: '#0000ff' }))
