...
let mainRoad = new T.TextureLoader().load(mainRoadTexture);
let roadSide = new T.TextureLoader().load(roadSideTexture);
let roadTurn = new T.TextureLoader().load(roadTurnTexture);
let roadSplit = new T.TextureLoader().load(roadSplitTexture);
...
/**@type {T.Vector2[]} */
var freeTreePositions = [];
/**@type {{vector: T.Vector2, tree: Tree}[]} */
var usedTreePositions = [];

let trainTrackPoints = [
...
export function grtown(renderer, canvasContainer) {
    const width = getComputedStyle(canvasContainer).getPropertyValue('width').slice(0, -2);
    const ratio = 3 / 4;
    // make the world
    ...
    let sphere = new T.SphereBufferGeometry(1);
    let mesh = new T.Mesh(sphere, new T.MeshStandardMaterial())
    ...
    for (var i = 0; i < 10; i++) {
        ...
        let tree = new Tree(((Math.floor(Math.random() * 2) + 1) / 2) + 2);
    ...
    for (var i = 0; i < numWalls; i++) {
        let height = 1;
        let wall = new BrickWall(1, height, 0.5);
    ...
    //helper variables
    var heliGoingToPoint = null;
    var heliHasTree = null
    var heliTree = null;
    let heli1 = new helicopter(1);
    heli1.objects[0].translateZ(5);
    heli1.objects[0].translateX(5);
    heli1.tick = (delta) => {

        let rotor = heli1.objects[0].children.find((obj) => obj.name == "rotor");

        let tailRotor = heli1.objects[0].children.find((obj) => obj.name == "tail").children.find((obj) => obj.name == "rotor");
        ...
        if (heliGoingToPoint) {
            ...
            let rotation = calculateRotation(positionChange);
            let speed = 40
            ...
            heli1.objects[0].position.set(newPosition.x, heli1.objects[0].position.y, newPosition.y)
            //helicopter has stopped at a point
            if (Math.abs(positionChange.x) < 0.5 && Math.abs(positionChange.y) < 0.5) {
                //path complete
                if (heliHasTree) {
                ...
                //picked up tree, heading to place it
                    ...
                    let point = getRandomFreePoint();
                    setPositionFree(heliGoingToPoint.x, heliGoingToPoint.z);
                    heliGoingToPoint = new T.Vector3(point.x, heli1.objects[0].position.y, point.y);
                    heliTree.objects[0].translateY(-100000);
                    heliHasTree = true;
                }
            }
        ...
        if (!heliGoingToPoint && Math.floor(performance.now()) % 10 == 0) {
            let point = getRandomUsedPoint();
            heliGoingToPoint = new T.Vector3(point.vector.x, heli1.objects[0].position.y, point.vector.y);
            heliTree = point.tree;
        }
    }
    world.add(heli1);
    let curve = buildTrainTrack(world);
    let train = new Train("#658575", curve);
    world.add(train);
    ...
    let cube = new T.CubeTextureLoader().load([
...
function getRandomFreePoint() {
    ...
    let randomIndex = Math.floor(Math.random() * freeTreePositions.length);
...
function getRandomUsedPoint() {
    //random is from 0 to not including 1, so .length is fine
    let randomIndex = Math.floor(Math.random() * usedTreePositions.length);
...
function setPositionUsed(x, y, tree) {
    ...
    usedTreePositions.push({ vector: freeTreePositions.splice(indexOf, 1)[0], tree: tree });
}
/**
 * O(N)
 * 
...
function buildRoad(world) {
    let roadBlock = new T.CubeGeometry(3.5, 0.05, 3.5);
    let sideBlock = new T.CubeGeometry(3.5, 0.05, 2.5);
    let backRoadPoints = [];
    for (var i = -1; i < 5; i++) {
        let roadMesh = new T.Mesh(roadBlock, new T.MeshStandardMaterial({ map: mainRoad }));
        let sideMeshR = new T.Mesh(sideBlock, new T.MeshStandardMaterial({ map: roadSide }));
        let sideMeshL = new T.Mesh(sideBlock, new T.MeshStandardMaterial({ map: roadSide }));
    ...
    roadPoints.forEach((point, index) => {
        backRoadPoints[roadPoints.length - 1 - index] = new T.Vector2(point.x, point.y + 0.5);
    })
    let loopByHouses = buildRoadLoop(roadBlock);
    backRoadPoints.forEach((point) => {
        roadPoints.push(point);
    })

    let roadMesh = new T.Mesh(roadBlock, new T.MeshStandardMaterial({ map: roadTurn }));
    let sideMeshR = new T.Mesh(sideBlock, new T.MeshStandardMaterial({ map: roadSide }));
    let sideMeshL = new T.Mesh(sideBlock, new T.MeshStandardMaterial({ map: roadSide }));
    ...
    let groupRoad = new T.Group();
    let groupSideR = new T.Group();
    let groupSideL = new T.Group();
    let tempPoints = [];
    for (var i = -1; i < 5; i++) {
        let roadMesh = new T.Mesh(roadBlock, new T.MeshStandardMaterial({ map: mainRoad }));
        let sideMeshR = new T.Mesh(sideBlock, new T.MeshStandardMaterial({ map: roadSide }));
        ...
        roadMesh.translateX(-i * 3.5);
        roadMesh.translateZ(5);
        let roadPointAt = new T.Vector2(7.5, (-i * 3.5) - 4);
    ...
    tempPoints.forEach((point, index) => {
        backRoadPoints[tempPoints.length - 1 - index] = new T.Vector2(point.x - 1, point.y);
    })
    let sideMeshLE1 = new T.Mesh(sideBlock, new T.MeshStandardMaterial({ map: roadSide }));
    sideMeshLE1.translateX(-5 * 3.5);
    sideMeshLE1.translateZ(8);
    groupSideL.add(sideMeshLE1);
    let sideMeshLE2 = new T.Mesh(new T.CubeGeometry(3, 0.05, 2.5), new T.MeshStandardMaterial({ map: roadSide }));
    sideMeshLE2.translateX(-5.8 * 3.5);
    sideMeshLE2.translateZ(8);
    groupSideL.add(sideMeshLE2);

    let mainRoadGroup = new T.Group();
    mainRoadGroup.add(groupRoad);
    mainRoadGroup.add(groupSideR);
    mainRoadGroup.add(groupSideL);
    let secondLoop = buildRoadLoop2(roadBlock);
...
function buildRoadLoop(roadBlock) {
    ...
    let roadMeshSplit = new T.Mesh(roadBlock, new T.MeshStandardMaterial({ map: roadSplit }));
    ...
    for (var i = 0; i < 4; i++) {
        ...
        let roadMesh1 = new T.Mesh(roadBlock, new T.MeshStandardMaterial({ map: roadTurn }));
        let xCor = -((5 + (i / 2 >= 1 ? 1 : 0)) * 3.5);
        let zCor = i / 2 >= 1 ? (i % 2 ? 8.5 : 1.5) : (i % 2 ? 1.5 : 8.5);
    ...
    let roadMeshFinish = new T.Mesh(roadBlock, new T.MeshStandardMaterial({ map: mainRoad }));
...
function buildRoadLoop2(roadBlock) {
    ...
    for (var i = 0; i < 4; i++) {
        ...
        group.add(roadMesh1);
        let vector = new T.Vector2(zCor + 8.75, -xCor - 12.5);
        let vector2 = new T.Vector2(zCor - 4.75, -xCor - 12.5);
        if (i == 0) {
...
function buildTrainTrack(world) {
    let cube = new T.CubeGeometry(1, 1, 1);

    let track = new T.Group();

    let trackPoints = [];
    let lastPoint = trainTrackPoints[trainTrackPoints.length - 1];
    trackPoints.push(new T.Vector2(lastPoint.x, lastPoint.z));
    trainTrackPoints.forEach(( /**@type {T.Vector3} */ point, index) => {
        let mesh = new T.Mesh(cube, new T.MeshStandardMaterial({ color: "#ff0000" }));
    ...
    var outerCurve = new T.SplineCurve(outerTrackPoints);
    var middleCurve = new T.SplineCurve(middleTrackPoints);
    let outerCurvePoints = outerCurve.getPoints(50);
    let innerCurvePoints = innerCurve.getPoints(50);
    let bufferPoints = new T.BufferGeometry().setFromPoints(innerCurvePoints);
    let bufferPoints2 = new T.BufferGeometry().setFromPoints(outerCurvePoints);

    let trackMesh = new T.Line(bufferPoints, new T.MeshStandardMaterial({ color: '#0000ff' }))
    let trackMesh2 = new T.Line(bufferPoints2, new T.MeshStandardMaterial({ color: '#0000ff' }));
...
function buildPost(height) {
    let ceil = Math.ceil(height);
    let group = new T.Group();
    for (var j = 0; j < ceil; j++) {
        let postSection = buildPostSection(height / ceil);
...
function buildPostSection(height) {
    let postMaterial = new T.MeshStandardMaterial({ color: "#663300" });
    let postSpindle = new T.CylinderBufferGeometry(0.1, 0.1, height);
    let group = new T.Group();
    let frontRightSpindle = new T.Mesh(postSpindle, postMaterial);
    let frontLeftSpindle = new T.Mesh(postSpindle, postMaterial);
    let backRightSpindle = new T.Mesh(postSpindle, postMaterial);
    let backLeftSpindle = new T.Mesh(postSpindle, postMaterial);
    let frontRightCross = buildPostCross(height);
    let frontLeftCross = buildPostCross(height);
    let backRightCross = buildPostCross(height);
    let backLeftCross = buildPostCross(height);
...
function buildPostCross(height) {
    let postMaterial = new T.MeshStandardMaterial({ color: "#663300" });
    let postSpindle = new T.CylinderBufferGeometry(0.1, 0.1);
    let group = new T.Group();

    let rightSpindle = new T.Mesh(postSpindle, postMaterial);
    let leftSpindle = new T.Mesh(postSpindle, postMaterial);
