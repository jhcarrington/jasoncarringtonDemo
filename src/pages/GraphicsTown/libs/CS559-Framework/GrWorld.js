...
export class GrWorld {
  ...
  constructor (params = {}) {
    ...
    console.log('renderer' in params, 'renderparams' in params ? params.renderparams : {})
    this.renderer =
        'renderer' in params
          ? params.renderer
          : new T.WebGLRenderer(
    ...
    if ('camera' in params) {
      ...
      if (!('lookfrom' in params)) {
        const gpSize = this.groundplane ? this.groundplane.size : 10
    ...
    if ('lights' in params) {
      ...
      const leftLight = params.sideLightColors
        ? params.sideLightColors[0]
        : leftColor;
      const rightLight = params.sideLightColors
        ? params.sideLightColors[1]
        : rightColor;

      const dirLight1 = new T.DirectionalLight(leftLight, params.lightBrightness)
      dirLight1.position.set(1, 1, -0.4);
      this.scene.add(dirLight1);

      const dirLight2 = new T.DirectionalLight(
        rightLight,
      ...
      const bottomLight = new T.DirectionalLight(
        bottomColor,
  ...
  orbitControlOn () {
    ...
    if (this.solo_mode && this.active_object) {
      const camparams = this.active_object.lookFromLookAt()
  ...
  followObjectOn () {
    if (this.active_object.rideable) {
      ...
      this.camera.position.set(
        0,
        bbox.max.y - bbox.min.y, -1.5 * (bbox.max.z - bbox.min.z)
      )
      ...
      // Set look direction
      const target = this.active_object.objects[0].position
  ...
  animate (callbacks = {}) {
    ...
    if (!this.runbutton || this.runbutton.checked) {
      ...
      const speed = this.speedcontrol ? Number(this.speedcontrol.value) : 1.0
